    ...
    public function update(Request $request, Room $room)
    {
        ...
        try {
            DB::transaction(function () use ($request, $room) {
                $updateData = [
                    'name' => $request->name,
                    'is_active' => $request->has('is_active') ? (bool) $request->is_active : $room->is_active,
                    'is_publish' => $request->has('is_publish') ? (bool) $request->is_publish : $room->is_publish,
                ];
                ...
                Log::info('Updating Room Data:', $updateData); // Log dữ liệu sẽ cập nhật
                $room->update($updateData);
    
                // Cập nhật trạng thái ghế nếu có trong request
                if ($request->has('seats')) {
                    foreach ($request->seats as $seatId => $seatData) {
                        Log::info("Updating Seat ID {$seatId}:", $seatData);
                        Seat::where('id', $seatId)->update(['is_active' => (bool) $seatData['is_active']]);
                    }
                }
            });
            ...
            Log::error('Room Update Error:', ['error' => $th->getMessage()]);
